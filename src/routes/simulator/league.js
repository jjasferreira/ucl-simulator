			...
			while (this.pots[i].length > 0) {
				...
				for (let j = i; j < NUMBER_POTS; j++) {
					...
					while (t1.fixtures.filter((f) => f.pot === j + 1).length < 2) {
						...
						DEBUG && console.log('generateFixtures(): drawn team =', t2.name, t2.pot, loc);
					}
				}
			...
			while (match < FIXTURES_PER_WEEK) {
				...
				if (availableFixtures.length === 0) {
					if (weekTries > 1000) {
						...
						week = 0;
						DEBUG && console.log('scheduleFixtures(): resetting week =', week + 1);
					}
					weeks[week] = [];
					playingTeams.clear();
					match = 0;
				...
				DEBUG && console.log('scheduleFixtures(): available fixtures =', availableFixtures);
