...
class Team {
  ...
  constructor(id, name, ground, design, country, primary, secondary, coefficient, pot) {
    this.id = id;
    this.name = name;
    this.ground = ground;
    this.design = design;
...
export class League {
  ...
  constructor(ids, teams) {
    this.teams = new Map();
    ids.forEach(id => this.teams.set(id, new Team(id, teams[id].name, teams[id].ground, teams[id].design, teams[id].country, teams[id].primary, teams[id].secondary, teams[id].coefficient)));
    DEBUG && console.log("league.teams", this.teams);

    this.pots = [...Array(NUMBER_POTS)].map(() => []);
    ids.forEach((id, idx) => { const pot = Math.floor(idx / (ids.length / NUMBER_POTS)) + 1; this.teams.get(id).pot = pot; this.pots[pot - 1].push(id); });
    DEBUG && console.log("league.pots", this.pots);

    this.table = [...ids];
    this.table.sort((a, b) => this.teams.get(a).name.localeCompare(this.teams.get(b).name));
    DEBUG && console.log("league.table", this.table);
